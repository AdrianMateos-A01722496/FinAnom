"""Entrenamiento del modelo FINANOM — pipeline HIBRIDO unificado y autonomo.
...
1. Isolation Forest no supervisado sobre las 63 features `feat_*`.
   - Muestreo ESTRATIFICADO y umbral ADAPTATIVO reutilizados de la copia local.
2. Reglas de negocio TIPADAS, reutilizando la copia local del motor de Tony
   (8 detectores con catalogos de codigos, scoring y mensajes legibles) + la regla de
   metodo de pago (Visa<->Amex) que aporta Adrian.
3. Explicabilidad SHAP (TreeExplainer, reutilizada desde la copia local de Rogelio) sobre las
...
feedback del auditor ("el modelo que aprende") vive en `model_final/feedback.py`.
...
from model_final.feedback import init_label_store  # noqa: E402
from model_final.reglas import detectar_anomalias  # noqa: E402
from model_final.rogelio_train import (  # noqa: E402
    compute_shap,
    pick_threshold,
    stratified_sample,
)
...
TRAINING_DATA_DIR = HERE / "training_data"
...
MODEL_CARD_FILE = HERE / "modelo_card.md"
...
# El bucle de feedback del auditor ("el modelo que aprende") vive en model_final/feedback.py
...
def write_eval_report(report, rules, injection, stability, overlap, threshold_ss,
    ...
    L.append("Generado por `model_final/train_model.py`. Consolida IF (muestreo "
...
def write_model_card(report, feature_cols, threshold_ss, paths: TrainingPaths) -> None:
    ...
    L.append("- **Isolation Forest** no supervisado con **muestreo estratificado** y "
             "**umbral adaptativo** (copia local de Rogelio).")
    L.append("- **Reglas de negocio tipadas**: motor local de reglas de Tony "
             "(8 detectores) + regla de **metodo de pago** Visa<->Amex (Adrian).")
    L.append("- **Explicabilidad SHAP** (TreeExplainer, copia local de Rogelio) sobre las filas del IF.\n")

    L.append("## 1. Entrada\n")
    L.append(f"- Matriz: `model_final/training_data/X_modelo.parquet` ({n:,} filas × {len(feature_cols)} `feat_*`).")
    L.append("- Reglas: base limpia `model_final/data/transacciones_limpio.parquet` (columnas crudas).")
    L.append("- Trazabilidad: `trace_*` de `model_final/training_data/transacciones_modelado.parquet`.\n")
    ...
    L.append("Primitivas en `model_final/feedback.py` (mecanismo de **adaptacion de umbral/pesos**): "
             "`feedback_metrics()` mide precision, `suggest_threshold()` ajusta el umbral del IF y "
             "`suggest_rule_weights()` aprende un factor de confianza por regla. El lazo COMPLETO "
             "(aplicar lo aprendido y regenerar la cola, sin reentrenar) vive en `model_final/` y se "
             "presenta en el dashboard.\n")
